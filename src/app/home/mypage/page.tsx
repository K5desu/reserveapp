      ...
      <section className="text-center border border-gray-300 p-4 max-w-sm w-full">
        ...
        {reserveData && (
          <div className="border border-gray-300 p-4 mt-4">
            <p>
              予約日：{reserveData.date.getFullYear()}年{" "}
              {reserveData.date.getMonth() + 1}月 {reserveData.date.getDate()}日
            </p>
            <p>
              予約時間：{reserveData.starttime.getHours()}時
              {reserveData.starttime.getMinutes()}分 ~{" "}
              {reserveData.finishtime.getHours()}時
              {reserveData.finishtime.getMinutes()}分
            </p>
            <p>予約部屋：{reserveData.room_number}</p>
          </div>
        )}
        <button
          onClick={() => removeReserve(author_id!)}
